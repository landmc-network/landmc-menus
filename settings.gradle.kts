...
rootProject.name = "landmc-menus"

// Two modules. menus-api is the wire format a menu's contents travel in; menus-paper is the
// plugin that draws them.
//
// There is deliberately no menus-proxy. The data a menu shows - a friend list, a punishment
// history - belongs to the plugin that already owns it, so those plugins send the payload
// themselves and depend on menus-api. A third module here would mean a second copy of every
// repository, reading the same tables behind the back of their owner.
include("menus-api", "menus-paper")
